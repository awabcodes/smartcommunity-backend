...
export class DonationRequestResolve implements Resolve<IDonationRequest> {
  constructor(private service: DonationRequestService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IDonationRequest> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((donationRequest: HttpResponse<DonationRequest>) => {
          if (donationRequest.body) {
            return of(donationRequest.body);
          } else {
...
export const donationRequestRoute: Routes = [
  {
    ...
    data: {
      ...
      defaultSort: 'id,asc',
      pageTitle: 'smartcommunityApp.donationRequest.home.title',
    },
